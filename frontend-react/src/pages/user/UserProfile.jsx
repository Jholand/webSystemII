...
const UserProfile = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (!token) {
          console.warn('No authentication token found. User needs to log in again.');
        ...
        if (storedUserId) {
          ...
          // Check for approved correction requests (optimized - single call)
          const response = await correctionRequestAPI.getUserRequests(storedUserId);
          const allRequests = response?.data || response || [];
          
          const userApprovedRequest = Array.isArray(allRequests) ? allRequests.find(
  ...
  // Poll for approved correction requests every 30 seconds (optimized)
  useEffect(() => {
    const checkApprovals = async () => {
      ...
      if (storedUserId) {
        try {
          ...
          const allNotifications = notificationsResponse?.data || notificationsResponse || [];
          
          setNotifications(allNotifications);
          
          const unreadNotifications = Array.isArray(allNotifications) ? allNotifications.filter(n => !n.read) : [];
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold" style={{ color: '#4158D0' }}>My Profile</h1>
              ...
              {!isEditing ? (
                <>
                  ...
                  <button
                    ...
                    style={{ 
                      background: approvedFields.length === 0 ? '#9CA3AF' : 'linear-gradient(135deg, #4158D0 0%, #2563eb 100%)',
                      boxShadow: approvedFields.length === 0 ? '' : '0 4px 12px rgba(65, 88, 208, 0.3)'
                    }}
                ...
                <>
                  ...
                  <button
                    onClick={handleSave}
                    className="px-4 py-2 text-sm font-medium text-white rounded-xl hover:opacity-90 transition-all shadow-lg"
                    style={{ 
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <h2 className="text-lg font-semibold mb-4" style={{ color: '#4158D0' }}>Member Information (Read-Only)</h2>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-xl flex items-start gap-2 border border-blue-100">
            <AlertCircle size={20} className="mt-0.5 flex-shrink-0" style={{ color: '#4158D0' }} />
